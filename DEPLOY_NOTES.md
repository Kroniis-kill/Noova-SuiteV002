...
## 7. Diseño visual / coherencia (esta pasada)
- **Causa raíz encontrada**: `index.html` cargaba un script de respaldo de Tailwind (`cdn.tailwindcss.com`) con su propia paleta de colores duplicada — y en el caso de "accent", con un valor DISTINTO al real (`#B621FF` morado vs el `brand-accent` real `#FF1493` rosa). 13-14 componentes de alta visibilidad (dashboard, resumen financiero, cuentas, login) dependían por completo de ese script externo para pintar sus colores. Se corrigieron todos para usar los tokens reales, se agregaron alias `shadow-glow`/`shadow-glow-sm` al `tailwind.config.js` real, y se eliminó el script del CDN de `index.html`.
- **Se formalizaron ~1100 usos de colores** que ya eran visualmente correctos pero usaban clases crudas de Tailwind (`text-red-500`, `bg-emerald-400`, etc.) en vez de los tokens semánticos del sistema (`status-danger`, `status-success-soft`, etc.). Todos los reemplazos se verificaron matemáticamente contra los valores RGB exactos de Tailwind antes de aplicarse — **cero cambio visual**, solo que ahora el color vive en un token centralizado en vez de estar repetido en cientos de lugares.
- Se descubrió y formalizó un **quinto estado semántico real** que la app ya usaba de forma consistente (`orange`, para "vence hoy") pero que no tenía token — se agregó `status-expiring`/`status-expiring-soft` en vez de forzarlo dentro de warning/danger y perder esa distinción.
- Se verificó con un script exhaustivo que los 162 archivos `.tsx` del proyecto siguen con llaves y paréntesis balanceados tras todos los reemplazos, y que ningún import quedó roto.
- **Pendiente de esta área**: variantes raras y aisladas de color (menos de 10 casos cada una, ej. `bg-red-900`, `text-blue-200`) que no siguen el patrón sistemático — se dejaron intactas por ser, muy probablemente, decisiones puntuales de contraste, no errores. También queda pendiente una revisión de tipografía, espaciado y radios de borde entre pantallas — no llegué a esa parte en esta pasada.
