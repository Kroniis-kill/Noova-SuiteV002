    ...
    >
        ...
        <div className="flex justify-between items-start mb-5 pl-3">
            <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-sm bg-surface-sunken flex items-center justify-center border border-white/5 shrink-0">
                    <Layers size={18} className="text-zinc-400" />
                </div>
                <div>
                  <h3 className="text-base font-bold text-white leading-tight">{service.service_name || 'Servicio'}</h3>
        ...
        <div className="bg-surface-sunken rounded-lg p-4 border border-white/5 space-y-3 mb-5 ml-2">
            <div className="flex justify-between items-center group cursor-pointer" onClick={() => copyToClipboard(user || '')}>
              <div className="flex flex-col min-w-0 pr-2">
                  <span className="text-[9px] font-bold text-zinc-500 uppercase">Usuario</span>
                  <span className="text-xs font-medium text-white truncate">{user || '---'}</span>
              </div>
              <Copy size={14} className="text-zinc-600 group-hover:text-white transition-colors shrink-0" />
            </div>
            ...
            <div className="flex justify-between items-center group cursor-pointer" onClick={() => copyToClipboard(pass || '')}>
              <div className="flex flex-col min-w-0 pr-2">
                  <span className="text-[9px] font-bold text-zinc-500 uppercase">Contraseña</span>
            ...
            {(pName || pPin) && (
                <>
                  ...
                  <div className="flex gap-4 pt-1">
                    {pName && (
                        <div className="flex-1 bg-surface-1 rounded-md p-3 border border-white/5">
                            <span className="text-[9px] font-bold text-zinc-500 uppercase block mb-1">Perfil</span>
                            <span className="text-xs font-semibold text-white truncate block">{pName}</span>
                        </div>
                    )}
                    {pPin && (
                        <div className="w-20 bg-surface-1 rounded-md p-3 border border-white/5 text-center">
                            <span className="text-[9px] font-bold text-zinc-500 uppercase block mb-1">PIN</span>
        ...
        <div className="flex items-center justify-between pl-3">
            ...
            <button onClick={() => onReport(service)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-sm bg-white/5 hover:bg-white/10 text-zinc-300 font-semibold text-[10px] transition-colors border border-white/5">
...
const PortalPage: React.FC = () => {
  ...
  if (view === 'loading') {
    return (
      <div className="min-h-screen bg-bg flex flex-col items-center justify-center">
         ...
         <p className="text-zinc-500 text-xs mt-6 font-medium animate-pulse">Conectando seguro...</p>
      </div>
    );
  }
  ...
  if (view === 'error') {
    return (
      <div className="min-h-screen bg-bg flex flex-col items-center justify-center p-6 text-center">
         ...
         <button onClick={() => window.location.reload()} className="mt-8 px-6 py-3 bg-white/10 rounded-full text-white font-semibold text-xs hover:bg-white/20 transition-colors border border-white/5 flex items-center gap-2">
            ...
            <div className="flex flex-col items-center w-full max-w-xs mx-auto animate-fade-in py-6">
                <div className="mb-6 text-center">
                    <div className="w-20 h-20 bg-gradient-to-tr from-brand-primary to-brand-accent rounded-xl flex items-center justify-center mx-auto mb-6 shadow-glow p-px">
                        <div className="w-full h-full bg-bg rounded-xl flex items-center justify-center">
                            {isSetup ? <Unlock className="w-8 h-8 text-white" /> : <Lock className="w-8 h-8 text-white" />}
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-3 gap-3 w-full mb-6 select-none">
                    {[1,2,3,4,5,6,7,8,9].map(n => (
                        <button key={n} onClick={() => handleNum(n.toString())} className="h-16 rounded-xl bg-surface-3 text-2xl font-medium text-white hover:bg-white/10 active:scale-90 transition-all border border-white/5 shadow-sm">{n}</button>
                    ))}
                    <div />
                    <button onClick={() => handleNum('0')} className="h-16 rounded-xl bg-surface-3 text-2xl font-medium text-white hover:bg-white/10 active:scale-90 transition-all border border-white/5 shadow-sm">0</button>
                    <button onClick={handleBackspace} className="h-16 rounded-xl flex items-center justify-center text-zinc-400 hover:text-white active:scale-90 transition-all"><ChevronRight className="rotate-180" size={28} /></button>
                </div>
  ...
  return (
      <div className="h-screen bg-bg font-sans text-zinc-100 flex flex-col selection:bg-brand-primary/30">
          {/* Header */}
          <div className="relative pt-8 pb-12 px-6 bg-surface-1 rounded-b-2xl border-b border-white/10 shadow-2xl z-10 shrink-0">
              ...
              <div className="relative z-10 flex justify-between items-center mb-6">
                  <div className="flex items-center gap-2">
                     ...
                     <span className="text-sm font-bold text-white tracking-widest uppercase">{branding?.name || 'Portal'}</span>
                  </div>
                  <button onClick={handleLogout} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-zinc-400 hover:text-white transition-colors border border-white/5">
                      <LogOut size={18} />
                  </button>
              </div>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar px-5 -mt-6 pt-10 pb-12 relative z-0">
              {services.length === 0 ? (
                  <div className="bg-surface-3 border border-white/10 rounded-xl p-10 text-center flex flex-col items-center shadow-lg mt-4">
                      <WifiOff size={32} className="text-zinc-600 mb-4" />
                      <p className="text-zinc-300 font-bold text-sm">Sin Servicios Activos</p>
                      <p className="text-zinc-500 text-xs mt-1">Contacta a soporte si crees que es un error.</p>
                  </div>
              ) : (
              ...
              <div className="mt-12 text-center opacity-40">
                 <p className="text-[10px] text-zinc-600 font-medium uppercase tracking-widest">Powered by Noova Suite</p>
              </div>
          </div>
      </div>
  );
