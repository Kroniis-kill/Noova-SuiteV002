...
const LegalPage: React.FC<LegalPageProps> = ({ type }) => {
  ...
  return (
    <div className="min-h-screen bg-bg text-primary font-sans selection:bg-brand-primary/30">
      ...
      <div className="sticky top-0 z-50 bg-bg/80 backdrop-blur-xl border-b border-[rgb(var(--fg-rgb))]/10">
         <div className="max-w-3xl mx-auto px-6 py-4 flex items-center gap-4">
             <button 
               onClick={goBack}
               className="w-10 h-10 rounded-sm bg-[rgb(var(--fg-rgb))]/5 border border-[rgb(var(--fg-rgb))]/5 flex items-center justify-center text-muted hover:text-primary hover:bg-[rgb(var(--fg-rgb))]/10 transition-colors active:scale-95"
             >
                <ArrowLeft size={20} />
             </button>
      ...
      <div className="max-w-3xl mx-auto px-6 py-10 pb-32">
         ...
         <div className="mb-10 text-center">
             <div className="w-20 h-20 bg-gradient-to-br from-brand-primary to-brand-accent rounded-xl mx-auto flex items-center justify-center shadow-glow mb-6">
                 {type === 'terms' ? <FileText size={40} className="text-primary" /> : <ShieldCheck size={40} className="text-primary" />}
             </div>
             <h2 className="text-3xl font-bold text-primary mb-2">{title}</h2>
             <p className="text-disabled text-sm">Última actualización: {updated}</p>
         </div>
