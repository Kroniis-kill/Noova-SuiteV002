...
const AdminAnalyticsDesktop: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const StatCard = ({ title, value, icon: Icon, color }: any) => (
     <div className="bg-surface-zinc/60 backdrop-blur-xl border border-[rgb(var(--fg-rgb))]/10 rounded-xl p-5 flex items-center gap-4">
        <div className={`w-12 h-12 rounded-md flex items-center justify-center ${color} bg-[rgb(var(--fg-rgb))]/5`}>
           <Icon size={24} />
        </div>
        <div>
