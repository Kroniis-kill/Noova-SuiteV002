    ...
    <div className="space-y-6">
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-4 bg-surface-sunken p-4 rounded-xl border border-white/5">
              <div 
                onClick={() => fileInputRef.current?.click()}
                className="w-20 h-20 rounded-lg bg-surface-3 border border-white/10 flex items-center justify-center overflow-hidden cursor-pointer relative group active:scale-95 transition-transform"
              >
                {image_url ? (
              ...
              <div className="flex-1">
                 <h4 className="text-white font-bold text-sm leading-tight">Logo o Icono</h4>
                 <p className="text-zinc-500 text-[10px] mt-1">Sube una imagen cuadrada de alta calidad para identificar mejor el servicio.</p>
                 <input 
                    type="file" 
                    ref={fileInputRef} 
    ...
    <div className="min-h-screen pb-40">
      <div className="relative z-20 flex items-center justify-between mb-4 px-2">
         <div>
            ...
            <p className="text-zinc-400 text-[11px] font-medium mt-0.5">Gestión de plataformas y precios</p>
         </div>
         <button 
      ...
      <div className="mb-6 relative z-20 px-1">
          <div className="relative h-[48px] bg-surface-3 border border-white/10 rounded-md flex items-center px-4 transition-all focus-within:border-brand-primary/50 shadow-sm">
              <Search size={18} className="text-zinc-500 shrink-0" />
              <input 
                 value={searchTerm} 
                 onChange={e => setSearchTerm(e.target.value)} 
                 placeholder="Buscar servicio..." 
                 className="bg-transparent border-none outline-none text-sm text-white w-full ml-3 placeholder:text-zinc-600 font-medium" 
              />
              {searchTerm && (
                ...
                <div className="flex justify-between items-start mb-4">
                   <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-zinc-500 uppercase font-semibold mt-0.5">{svc.screens} Cupos • {svc.type.replace('_', ' ')}</p>
                      </div>
                   </div>
