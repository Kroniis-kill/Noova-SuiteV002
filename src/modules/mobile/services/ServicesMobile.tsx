...
const ServicesMobile: React.FC<ServicesMobileProps> = ({ onBack }) => {
  ...
  const inputClass = "w-full bg-surface-sunken border border-[rgb(var(--fg-rgb))]/10 rounded-md pl-11 pr-4 py-4 text-primary text-sm outline-none focus:border-brand-primary transition-all font-medium";
  const labelClass = "text-[10px] font-semibold text-disabled uppercase tracking-widest mb-2 block ml-1";
  ...
  const renderFields = () => (
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-4 bg-surface-sunken p-4 rounded-xl border border-[rgb(var(--fg-rgb))]/5">
              ...
              >
                {image_url ? (
                  ...
                  <div className="flex flex-col items-center gap-1 text-disabled">
                    <Camera size={24} />
              ...
              <div className="flex-1">
                 <h4 className="text-primary font-bold text-sm leading-tight">Logo o Icono</h4>
                 <p className="text-disabled text-[10px] mt-1">Sube una imagen cuadrada de alta calidad para identificar mejor el servicio.</p>
                 <input 
                    type="file" 
                    ref={fileInputRef} 
      ...
      <div className="bg-surface-sunken rounded-xl p-5 border border-[rgb(var(--fg-rgb))]/5 space-y-4">
        <h4 className="text-[11px] font-semibold text-muted uppercase flex items-center gap-2"><Calculator size={14} className="text-brand-primary" /> Estructura de Costos</h4>
        <div>
        ...
        <div className="bg-brand-primary/10 border border-brand-primary/20 rounded-md p-4 flex items-center justify-between">
            ...
            <span className="text-lg font-bold text-primary">${calculatedCost.toFixed(2)}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen pb-40">
      <div className="relative z-20 flex items-center justify-between mb-4 px-2">
         <div>
            <h1 className="text-2xl font-bold text-primary tracking-tight">Catálogo</h1>
            <p className="text-muted text-[11px] font-medium mt-0.5">Gestión de plataformas y precios</p>
         </div>
         <button 
      ...
      <div className="mb-6 relative z-20 px-1">
          <div className="relative h-[48px] bg-surface-3 border border-[rgb(var(--fg-rgb))]/10 rounded-md flex items-center px-4 transition-all focus-within:border-brand-primary/50 shadow-sm">
              ...
              <input 
                 ...
                 className="bg-transparent border-none outline-none text-sm text-primary w-full ml-3 placeholder:text-faint font-medium" 
              />
              {searchTerm && (
                 <button onClick={() => setSearchTerm('')} className="p-1">
                     <X size={14} className="text-disabled" />
                 </button>
              )}
          </div>
      </div>
              ...
              <motion.div key={svc.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }} transition={{ delay: idx * 0.05 }} className="bg-surface-3 border border-[rgb(var(--fg-rgb))]/[0.08] rounded-xl p-5 shadow-lg relative overflow-hidden">
                <div className="flex justify-between items-start mb-4">
                   <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="text-primary font-bold text-base leading-tight">{svc.name}</h4>
                        <p className="text-[10px] text-disabled uppercase font-semibold mt-0.5">{svc.screens} Cupos • {svc.type.replace('_', ' ')}</p>
                      </div>
                   </div>
                   <div className="flex gap-2">
                      <button onClick={() => handleEditClick(svc)} className="w-9 h-9 rounded-sm bg-[rgb(var(--fg-rgb))]/5 flex items-center justify-center text-muted active:scale-90 transition-all"><Edit2 size={15} /></button>
                ...
                <div className="grid grid-cols-3 gap-2 pt-4 border-t border-[rgb(var(--fg-rgb))]/5">
                   <div className="text-center"><p className="text-[9px] font-bold text-disabled uppercase">Costo</p><p className="text-secondary font-bold text-sm mt-0.5">${svc.cost || 0}</p></div>
                   <div className="text-center"><p className="text-[9px] font-bold text-disabled uppercase">Venta</p><p className="text-status-success-soft font-bold text-sm mt-0.5">${svc.publicPrice || 0}</p></div>
                   <div className="text-center"><p className="text-[9px] font-bold text-disabled uppercase">Utilidad</p><p className="text-brand-primary font-bold text-sm mt-0.5">${((svc.publicPrice || 0) - (svc.cost || 0)).toFixed(1)}</p></div>
                </div>
              </motion.div>
            ))}
        </AnimatePresence>
