...
const DashboardProMobile: React.FC<DashboardProMobileProps> = ({ setView }) => {
  ...
  const stats = [
    ...
    { label: 'Clientes', value: clients.length, icon: Users, color: 'text-blue-400', bg: 'bg-blue-500/10' },
    { label: 'Stock Disp.', value: accounts.reduce((acc, a) => acc + (a.maxScreens - calculateOccupancy(a)), 0), icon: Layers, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
    { label: 'Vencimientos', value: stockUrgent, icon: Clock, color: 'text-red-400', bg: 'bg-red-500/10' },
  ];
       ...
       <div className="px-6 space-y-6">
          <div className="bg-surface-3 border border-white/10 rounded-2xl p-6 relative overflow-hidden shadow-2xl">
             ...
             <div className="flex flex-col items-center text-center">
                ...
                <div className="flex items-center gap-2 mt-4"><div className="px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full flex items-center gap-1.5"><ArrowUpRight size={14} className="text-emerald-400" /><span className="text-[11px] font-semibold text-emerald-400">Rendimiento Pro</span></div></div>
             </div>
             <div className="mt-8 pt-6 border-t border-white/5 flex justify-between">
                ...
                <div className="text-center px-4 flex-1"><p className="text-[9px] font-bold text-zinc-500 uppercase mb-1">Egresos Mes</p><p className="text-sm font-bold text-red-400">-{formatMoney(expenses.reduce((acc,e) => acc + e.amount, 0))}</p></div>
             </div>
          </div>
          ...
          <div className="space-y-3">
             ...
             <div className="bg-surface-3 border border-white/10 rounded-xl overflow-hidden">
                <button onClick={() => setView('expired')} className="w-full flex items-center justify-between p-5 border-b border-white/5 active:bg-white/5 transition-colors"><div className="flex items-center gap-4"><div className="w-10 h-10 rounded-md bg-red-500/10 text-red-500 flex items-center justify-center"><AlertTriangle size={20} /></div><div className="text-left"><h4 className="text-sm font-bold text-white">Renovaciones Críticas</h4><p className="text-[10px] text-zinc-500">{stockUrgent} clientes por vencer</p></div></div><ChevronRight size={18} className="text-zinc-600" /></button>
                <button onClick={() => setView('inventory')} className="w-full flex items-center justify-between p-5 active:bg-white/5 transition-colors"><div className="flex items-center gap-4"><div className="w-10 h-10 rounded-md bg-amber-500/10 text-amber-500 flex items-center justify-center"><Layers size={20} /></div><div className="text-left"><h4 className="text-sm font-bold text-white">Stock e Inventario</h4><p className="text-[10px] text-zinc-500">Gestión de cupos activos</p></div></div><ChevronRight size={18} className="text-zinc-600" /></button>
             </div>
          </div>
       </div>
       <NotificationCenter 
