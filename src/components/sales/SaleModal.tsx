...
const SaleModal: React.FC<SaleModalProps> = ({ isOpen, onClose, initialData, zIndex }) => {
  ...
  return (
    <>
      ...
      <SearchListModal isOpen={modalSearch === 'account'} onClose={() => setModalSearch(null)} items={accounts.filter(a => a.serviceId === tempServiceId && a.status === 'activa' && (a.maxScreens - calculateOccupancy(a)) > 0)} onSelect={(a: Account) => setTempAccountId(a.id)} title="Seleccionar Stock" filterFn={(a, q) => a.email.toLowerCase().includes(q)} zIndex={zIndex ? zIndex + 200 : undefined} renderItem={(a: Account) => (<div className="p-4 rounded-lg bg-surface-1 border border-[rgb(var(--fg-rgb))]/5 mb-1 flex justify-between items-center hover:border-brand-primary/40 transition-all"><div className="flex flex-col min-w-0 pr-3"><span className="text-sm font-bold text-text-primary truncate max-w-[200px] leading-tight">{a.email}</span><span className="text-[10px] font-mono text-text-disabled mt-1">Expira: {a.endDate}</span></div><div className="flex flex-col items-end"><span className={`text-[9px] font-black text-status-success-soft uppercase bg-status-success/10 px-2 py-1 rounded-lg border border-status-success/20 tracking-widest`}>Disponible</span><span className="text-[8px] text-text-faint font-bold mt-1">{a.maxScreens - calculateOccupancy(a)} LIBRES</span></div></div>)} />
