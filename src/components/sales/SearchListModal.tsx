      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="bg-surface-1 w-full max-w-md rounded-lg border border-border-subtle shadow-modal overflow-hidden flex flex-col max-h-[75dvh]" onClick={e => e.stopPropagation()}>
        <div className="p-4 border-b border-white/5 flex flex-col gap-3">
          <div className="flex justify-between items-center">
            <h3 className="text-white font-bold text-center ml-1">{title}</h3>
            <button onClick={onClose} className="p-1 rounded-full bg-white/5 hover:bg-white/10 active:scale-95 transition-transform"><X size={16} className="text-zinc-400" /></button>
          </div>
          <div className="relative">
            <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500" />
            <input autoFocus value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar..." className="w-full bg-surface-sunken rounded-md pl-11 pr-10 h-[52px] text-sm text-white outline-none border border-white/10 focus:border-brand-primary/50 transition-all font-medium" />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-2">
          ...
          {filtered.length === 0 && <p className="text-center text-zinc-500 text-sm py-8">No hay resultados.</p>}
        </div>
      </motion.div>
    </div>, document.body
  );
