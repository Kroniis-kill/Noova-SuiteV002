      ...
      <div className="space-y-6 pt-1 pb-4">
        ...
        {step === 'select' ? (
            <div className="space-y-6 animate-fade-in">
                ...
                <div className="bg-surface-3 border border-white/[0.08] rounded-xl p-4 flex items-center gap-4 shadow-sm">
                    ...
                    <div className="flex-1 min-w-0">
                        <h4 className="text-[15px] font-bold text-white truncate">{clientName}</h4>
                        <p className="text-[10px] text-zinc-500 font-mono tracking-tight">{clientPhone}</p>
                    </div>
                </div>
                ...
                <div className="space-y-3">
                   <div className="flex items-center justify-between px-1">
                      <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Servicios del Mensaje</span>
                         ...
                         return (
                            ...
                            >
                               <div className="flex items-center gap-3 min-w-0">
                                  <div className={`w-9 h-9 rounded-sm flex items-center justify-center shrink-0 ${isSelected ? 'bg-brand-primary/10 text-brand-primary' : 'bg-zinc-800 text-zinc-600'}`}>
                                      <Monitor size={18} />
                                  </div>
                ...
                <div className="space-y-3">
                    <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] ml-1">Elegir Plantilla</span>
                    <div className="grid grid-cols-1 gap-2.5">
                      {templateOptions.map((opt) => (
                        <button
                          ...
                          className="flex items-center gap-4 p-3.5 rounded-lg bg-surface-1 border border-white/5 hover:bg-surface-3 hover:border-white/10 active:scale-[0.98] transition-all group overflow-hidden relative disabled:opacity-20"
                        >
                            <div className={`w-9 h-9 rounded-sm flex items-center justify-center shrink-0 ${opt.bg} ${opt.color}`}>
                               <opt.icon size={18} />
                            </div>
                            <div className="flex-1 text-left">
                                <span className="block text-[13px] font-bold text-zinc-100 group-hover:text-white transition-colors">{opt.label}</span>
                                <span className="block text-[8px] font-bold text-zinc-600 uppercase tracking-[0.2em] mt-0.5">Plantilla del Sistema</span>
                            </div>
                            {opt.badge && (
                            ...
                            <ChevronRight size={16} className="text-zinc-700 opacity-0 group-hover:opacity-100 transition-opacity" />
                        </button>
                      ))}
            ...
            <div className="space-y-8 animate-fade-in">
                ...
                <div className="space-y-3">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] ml-1">Formato de Precios</label>
                    <div className="flex bg-surface-1 p-1.5 rounded-lg border border-white/[0.08] w-full shadow-lg">
                        <button 
                            ...
                            className={`flex-1 flex items-center justify-center gap-2.5 py-4 rounded-lg text-[11px] font-semibold uppercase tracking-widest transition-all ${!useSecondaryCurrency ? 'bg-white text-black shadow-[0_4px_15px_rgba(255,255,255,0.2)]' : 'text-zinc-500 hover:text-zinc-300'}`}
                        >
                ...
                <div className="space-y-3">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] ml-1">Opciones Adicionales</label>
                    <button 
                        onClick={() => setIncludeReceipt(!includeReceipt)}
                        className={`w-full p-4 rounded-xl border flex items-center justify-between transition-all duration-300 ${includeReceipt ? 'bg-brand-primary/10 border-brand-primary shadow-glow-sm' : 'bg-surface-1 border-white/5 hover:border-white/10'}`}
                    >
                        <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-lg flex items-center justify-center transition-all ${includeReceipt ? 'bg-brand-primary text-white shadow-glow' : 'bg-surface-sunken text-zinc-600'}`}>
                                <ImagePlus size={22} />
                            </div>
                            <div className="text-left">
                                <span className={`block text-sm font-bold ${includeReceipt ? 'text-white' : 'text-zinc-300'}`}>Comprobante Digital</span>
                ...
                <div className="pt-4 space-y-3">
                    ...
                    <button 
                        ...
                        className="w-full h-14 flex items-center justify-center gap-2 text-zinc-600 hover:text-white text-[10px] font-semibold uppercase tracking-[0.3em] transition-all active:scale-95"
                    >
