      ...
      <>
        ...
        >
            ...
            <div className="flex justify-between items-center w-full z-10 px-0.5">
                <div className="flex items-center gap-1 text-[9px] text-disabled font-medium bg-[rgb(var(--fg-rgb))]/[0.03] px-1.5 py-0.5 rounded-md">
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-1 mt-1">
                ...
                <div className="text-center w-full space-y-0.5">
                    <h3 className="text-[10px] font-semibold text-primary leading-tight tracking-tight line-clamp-2 px-1 not-italic">
                        {group.clientName}
                    </h3>
            ...
            <div className="flex items-center justify-between gap-2 mt-2 w-full shrink-0">
                <button 
                  ...
                  className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all active:scale-90 ${isUrgent ? 'bg-brand-whatsapp border-brand-whatsapp text-black shadow-glow-sm' : 'bg-[rgb(var(--fg-rgb))]/5 border-[rgb(var(--fg-rgb))]/[0.05] text-disabled hover:text-primary'}`}
                >
                   {isUrgent ? <BellRing size={12} fill="currentColor" /> : <MessageCircle size={12} />}
                </button>
                <button 
                  onClick={(e) => { e.stopPropagation(); onDelete(group); }} 
                  className="w-8 h-8 rounded-full bg-[rgb(var(--fg-rgb))]/5 text-faint border border-[rgb(var(--fg-rgb))]/[0.05] flex items-center justify-center hover:text-status-danger-soft transition-all active:scale-90"
                >
                   <Trash2 size={12} />
                </button>
            </div>
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-sm font-medium text-primary truncate leading-tight not-italic">{group.clientName}</h3>
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-muted flex items-center gap-1"><Layers size={12} /> {totalScreens} Cupos</span>
                    <span className="text-faint">•</span>
                    <span className={`font-medium ${timeColor}`}>{statusText}</span>
                  </div>
              </div>
              <div className="w-8 h-8 rounded-full bg-[rgb(var(--fg-rgb))]/5 flex items-center justify-center text-disabled group-hover:text-primary group-hover:bg-[rgb(var(--fg-rgb))]/10 transition-colors"><ChevronRight size={16} /></div>
            </div>
            <div className="flex items-center justify-between pt-3 border-t border-[rgb(var(--fg-rgb))]/[0.08]">
              <div className="flex flex-col">
                  <span className="text-[9px] font-semibold text-disabled uppercase tracking-wider">Total</span>
                  <span className="text-sm font-medium text-primary">${allSales.reduce((a,c)=>a+c.amount,0).toLocaleString()}</span>
              </div>
