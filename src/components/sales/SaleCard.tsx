      ...
      <>
        ...
        >
            ...
            <div className="flex justify-between items-center w-full z-10 px-0.5">
                <div className="flex items-center gap-1 text-[9px] text-zinc-500 font-medium bg-white/[0.03] px-1.5 py-0.5 rounded-md">
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-1 mt-1">
                <div className="relative">
                   <Avatar name={group.clientName} size={36} className="rounded-full shadow-md border border-white/5" />
                </div>
                
                <div className="text-center w-full space-y-0.5">
                    <h3 className="text-[10px] font-semibold text-white leading-tight tracking-tight line-clamp-2 px-1 not-italic">
                        {group.clientName}
                    </h3>
            ...
            <div className="flex items-center justify-between gap-2 mt-2 w-full shrink-0">
                <button 
                  ...
                  className={`w-8 h-8 rounded-full border flex items-center justify-center transition-all active:scale-90 ${isUrgent ? 'bg-brand-whatsapp border-brand-whatsapp text-black shadow-glow-sm' : 'bg-white/5 border-white/[0.05] text-zinc-500 hover:text-white'}`}
                >
        ...
        {showCurrencyModal && (
            <Modal isOpen={true} onClose={() => setShowCurrencyModal(false)} title="Enviar Aviso" zIndex={20000}>
              <div className="space-y-4 pt-2" onClick={e => e.stopPropagation()}>
                ...
                <div className="bg-surface-1 border border-white/[0.08] rounded-md p-4 text-center">
      ...
      >
        ...
        <div className="p-5 pl-7 relative z-10">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-zinc-500 group-hover:text-white group-hover:bg-white/10 transition-colors"><ChevronRight size={16} /></div>
            </div>
            <div className="flex items-center justify-between pt-3 border-t border-white/[0.08]">
              <div className="flex flex-col">
                  <span className="text-[9px] font-semibold text-zinc-500 uppercase tracking-wider">Total</span>
                  <span className="text-sm font-medium text-white">${allSales.reduce((a,c)=>a+c.amount,0).toLocaleString()}</span>
              </div>
