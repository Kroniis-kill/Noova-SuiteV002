  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Extender Periodo">
        <div className="space-y-6 pt-2">
            ...
            <div>
                <label className="text-xs font-semibold text-zinc-500 uppercase mb-2 block">Días a agregar</label>
                <div className="flex items-center gap-3">
                    <button onClick={() => setDays(Math.max(1, days - 1))} className="w-10 h-10 rounded-sm bg-white/5 hover:bg-white/10 text-white font-bold border border-white/10 flex items-center justify-center">-</button>
                    <div className="flex-1 h-10 bg-surface-sunken border border-white/10 rounded-sm flex items-center justify-center text-white font-bold text-lg">
                        {days}
                    </div>
                    <button onClick={() => setDays(days + 1)} className="w-10 h-10 rounded-sm bg-white/5 hover:bg-white/10 text-white font-bold border border-white/10 flex items-center justify-center">+</button>
                </div>
            </div>
            ...
            <div className="flex gap-2 justify-center py-2">
                {[1, 3, 7, 15, 30].map(d => (
                    <button 
                        key={d}
                        onClick={() => setDays(d)}
                        className={`px-3 py-1.5 rounded-sm text-[10px] font-semibold border transition-all ${days === d ? 'bg-brand-primary border-brand-primary text-white' : 'bg-white/5 border-white/5 text-zinc-400 hover:text-white'}`}
                    >
                        +{d}d
                    </button>
                ))}
