  ...
  return (
    <div className="bg-surface-sunken border border-white/[0.06] rounded-xl p-5 shadow-2xl relative overflow-hidden flex flex-col">
       ...
       <div className="flex justify-between items-start mb-6 relative z-10">
          ...
          <div className="flex bg-bg p-1 rounded-sm border border-white/[0.05]">
             {['week', 'month', 'year'].map((p) => (
                <button
                  key={p}
                  onClick={() => setPeriod(p as any)}
                  className={`px-3 py-1 rounded-xs text-[8px] font-black uppercase tracking-widest transition-all ${period === p ? 'bg-surface-4 text-white' : 'text-zinc-600 hover:text-zinc-400'}`}
                >
       ...
       <div className="grid grid-cols-3 gap-3 mb-6 relative z-10">
          <div className="bg-surface-3/40 rounded-lg p-3.5 border border-white/[0.03]">
             ...
             <p className="text-sm font-bold text-white font-mono">{currency}{summary.income.toLocaleString()}</p>
          </div>
          ...
          <div className="bg-brand-primary/5 rounded-lg p-3.5 border border-brand-primary/10">
             ...
             <p className="text-sm font-bold text-white font-mono">{currency}{summary.netProfit.toLocaleString()}</p>
          </div>
       </div>
       ...
       <div className="mt-2 pt-4 border-t border-white/[0.03] flex items-center justify-between relative z-10">
          <div className="flex items-center gap-3">
             ...
             <div>
                <p className="text-[8px] text-zinc-600 font-black uppercase tracking-[0.2em]">Margen Neto</p>
                <p className="text-sm font-bold text-white font-mono">{margin.toFixed(1)}%</p>
             </div>
          </div>
