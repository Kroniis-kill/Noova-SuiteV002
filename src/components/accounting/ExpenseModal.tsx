    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Registro de Movimiento">
       <div className="pt-2">
          <div className={styles.tabsContainer}>
             <button type="button" onClick={() => setTab('expense')} className={`${styles.tabButton} ${tab === 'expense' ? 'bg-surface-4 text-text-primary shadow-sm' : 'text-text-faint hover:text-text-muted'}`}>Gastos</button>
             <button type="button" onClick={() => setTab('supply')} className={`${styles.tabButton} ${tab === 'supply' ? 'bg-surface-4 text-text-primary shadow-sm' : 'text-text-faint hover:text-text-muted'}`}>Insumos</button>
          </div>
