...
const ExpenseModal: React.FC<ExpenseModalProps> = ({ isOpen, onClose, initialTab = 'expense' }) => {
  ...
  const styles = {
    label: "text-[10px] font-semibold text-disabled uppercase tracking-widest mb-1.5 block ml-1",
    ...
    iconElement: "absolute left-4 text-disabled pointer-events-none group-focus-within:text-brand-primary",
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Registro de Movimiento">
       <div className="pt-2">
          <div className={styles.tabsContainer}>
             <button type="button" onClick={() => setTab('expense')} className={`${styles.tabButton} ${tab === 'expense' ? 'bg-surface-4 text-primary shadow-sm' : 'text-faint hover:text-muted'}`}>Gastos</button>
             <button type="button" onClick={() => setTab('supply')} className={`${styles.tabButton} ${tab === 'supply' ? 'bg-surface-4 text-primary shadow-sm' : 'text-faint hover:text-muted'}`}>Insumos</button>
          </div>
          <form onSubmit={handleSubmit} className="max-w-4xl mx-auto space-y-6">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 border-t border-[rgb(var(--fg-rgb))]/5 pt-6">
                ...
                <div><label className={styles.label}>Medio de Pago</label><div className={styles.inputContainer}><CreditCard size={16} className={styles.iconElement} /><select className="w-full bg-transparent text-xs text-primary px-11 outline-none appearance-none cursor-pointer" value={method} onChange={e => setMethod(e.target.value)}><option value="efectivo">Efectivo</option><option value="transferencia">Banco</option><option value="binance">Cripto</option><option value="zelle">Zelle</option></select><ChevronDown size={14} className="absolute right-4 text-disabled" /></div></div>
             </div>
