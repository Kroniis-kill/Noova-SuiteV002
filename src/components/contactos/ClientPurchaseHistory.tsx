...
const ClientPurchaseHistory: React.FC<ClientPurchaseHistoryProps> = ({ clientId }) => {
  ...
  if (clientSales.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-12 text-zinc-500 opacity-60 bg-white/[0.01] border border-dashed border-white/5 rounded-2xl">
  ...
  return (
    <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
      {clientSales.map((sale) => {
        ...
        return (
          <div key={sale.id} className="group relative bg-surface-1 border border-white/[0.08] rounded-xl p-4 transition-all hover:bg-surface-3 hover:border-white/20 shadow-sm overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
               <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-surface-sunken border border-white/5 flex items-center justify-center text-brand-primary shadow-inner overflow-hidden relative group-hover:border-brand-primary/30 transition-colors">
                     {service?.image_url ? (
                  ...
                  <div className="min-w-0">
                     <h4 className="text-[14px] font-black text-white uppercase tracking-tight truncate mb-0.5">{sale.serviceName}</h4>
                     <p className="text-[11px] text-zinc-400 font-mono truncate selection:bg-brand-primary/30">{displayEmail}</p>
                     
                     <div className="flex items-center gap-2 mt-2">
                        <span className="px-1.5 py-0.5 rounded-md bg-white/5 text-[9px] font-bold text-zinc-500 uppercase border border-white/5">
            ...
            <div className="grid grid-cols-2 gap-4 mt-2 pt-4 border-t border-white/[0.03] relative z-10">
               <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center text-zinc-600 border border-white/5">
                      <Calendar size={14} />
                  </div>
                  <div className="min-w-0">
                     <p className="text-[8px] text-zinc-500 font-black uppercase tracking-widest">Adquirido</p>
                     <p className="text-[10px] text-zinc-200 font-bold truncate">{formatDate(sale.date)}</p>
                  </div>
               </div>
               <div className="flex items-center gap-2.5">
                  <div className="w-8 h-8 rounded-xl bg-white/5 flex items-center justify-center text-zinc-600 border border-white/5">
                  ...
                  <div className="min-w-0">
                     <p className="text-[8px] text-zinc-500 font-black uppercase tracking-widest">Vencimiento</p>
