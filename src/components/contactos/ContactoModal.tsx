           ...
           {filtered.map(r => (
              <button key={r.id} onClick={() => { onSelect(r); onClose(); }} className="w-full flex items-center gap-3 p-3 rounded-sm bg-surface-zinc/60 border border-[rgb(var(--fg-rgb))]/5 hover:bg-surface-4 hover:border-brand-primary/30 transition-all group text-left">
                 ...
                 <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-text-disabled font-mono">{r.code}</p>
                 </div>
...
const ContactoModal: React.FC<ContactoModalProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
  ...
  const styles = {
    ...
    textarea: "w-full bg-surface-sunken border border-[rgb(var(--fg-rgb))]/10 rounded-sm p-4 text-xs text-text-primary outline-none focus:border-brand-primary/60 font-medium resize-none",
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                <h4 className="text-text-faint text-[9px] font-black uppercase tracking-[0.2em] mb-3 flex items-center gap-2">Información de Perfil</h4>
              ...
              <div className="space-y-4">
                <h4 className="text-text-faint text-[9px] font-black uppercase tracking-[0.2em] mb-3 flex items-center gap-2">Estado y Notas</h4>
                <div className={`p-4 rounded-md border flex items-center justify-between transition-all ${formData.isBlocked ? 'bg-status-danger/10 border-status-danger/20' : 'bg-surface-zinc border-[rgb(var(--fg-rgb))]/5'}`}><div className="flex items-center gap-3"><div className={`w-9 h-9 rounded-full flex items-center justify-center ${formData.isBlocked ? 'bg-status-danger text-white shadow-glow-sm' : 'bg-[rgb(var(--fg-rgb))]/5 text-text-disabled'}`}><Ban size={16} /></div><div><h4 className={`text-xs font-semibold ${formData.isBlocked ? 'text-status-danger-soft' : 'text-text-secondary'}`}>{formData.isBlocked ? 'Bloqueado' : 'Acceso Libre'}</h4><p className="text-[9px] text-text-disabled leading-tight">Evita ventas accidentales.</p></div></div><button type="button" onClick={() => setFormData(prev => ({ ...prev, isBlocked: !prev.isBlocked }))} className={`w-10 h-5.5 rounded-full relative transition-colors duration-300 border ${formData.isBlocked ? 'bg-status-danger border-status-danger' : 'bg-zinc-800 border-zinc-700'}`}><div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-md transition-transform duration-300 ${formData.isBlocked ? 'translate-x-4' : ''}`} /></button></div>
