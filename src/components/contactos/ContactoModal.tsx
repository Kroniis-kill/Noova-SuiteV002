      ...
      <div className="flex flex-col h-[50vh] md:h-[350px] pt-1">
        <div className="relative mb-3 shrink-0">
           ...
           <input 
             ...
             className="w-full bg-surface-sunken border border-[rgb(var(--fg-rgb))]/10 rounded-sm pl-10 pr-10 h-10 lg:h-11 text-xs text-primary outline-none focus:border-brand-primary/50 transition-all placeholder:text-faint font-medium"
             autoFocus
           />
           {search && <button onClick={() => setSearch('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-disabled hover:text-primary p-1"><X size={14} /></button>}
        </div>
        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-1.5 pr-1">
           <button onClick={() => { onSelect(null); onClose(); }} className="w-full flex items-center gap-3 p-3 rounded-sm bg-surface-zinc/60 border border-[rgb(var(--fg-rgb))]/5 hover:bg-surface-4 transition-all group text-left">
              <div className="w-8 h-8 rounded-xs bg-[rgb(var(--fg-rgb))]/5 flex items-center justify-center text-muted border border-[rgb(var(--fg-rgb))]/5 shrink-0"><User size={14} /></div>
              <div className="flex-1 min-w-0"><p className="text-xs font-semibold text-secondary">Cliente Directo</p></div>
              <Check size={14} className="text-faint" />
           </button>
           {filtered.map(r => (
              <button key={r.id} onClick={() => { onSelect(r); onClose(); }} className="w-full flex items-center gap-3 p-3 rounded-sm bg-surface-zinc/60 border border-[rgb(var(--fg-rgb))]/5 hover:bg-surface-4 hover:border-brand-primary/30 transition-all group text-left">
                 <div className="w-8 h-8 rounded-xs flex items-center justify-center text-primary font-semibold text-[10px] shadow-lg shrink-0" style={{ backgroundColor: r.color }}>{r.name.substring(0,2).toUpperCase()}</div>
                 <div className="flex-1 min-w-0">
                    <p className="text-xs font-semibold text-secondary group-hover:text-primary truncate">{r.name}</p>
                    <p className="text-[9px] text-disabled font-mono">{r.code}</p>
                 </div>
...
const ContactoModal: React.FC<ContactoModalProps> = ({ isOpen, onClose, onSubmit, initialData }) => {
  ...
  const styles = {
    ...
    inputContainer: "relative flex items-center bg-surface-sunken border border-[rgb(var(--fg-rgb))]/10 rounded-sm h-10 lg:h-11 transition-all focus-within:border-brand-primary/60",
    input: "w-full h-full bg-transparent text-xs text-primary placeholder:text-faint px-4 outline-none font-medium",
    iconLeft: "pl-11",
    iconElement: "absolute left-4 text-disabled pointer-events-none",
    textarea: "w-full bg-surface-sunken border border-[rgb(var(--fg-rgb))]/10 rounded-sm p-4 text-xs text-primary outline-none focus:border-brand-primary/60 font-medium resize-none",
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="space-y-4">
                <h4 className="text-faint text-[9px] font-black uppercase tracking-[0.2em] mb-3 flex items-center gap-2">Información de Perfil</h4>
              ...
              <div className="space-y-4">
                <h4 className="text-faint text-[9px] font-black uppercase tracking-[0.2em] mb-3 flex items-center gap-2">Estado y Notas</h4>
                <div className={`p-4 rounded-md border flex items-center justify-between transition-all ${formData.isBlocked ? 'bg-status-danger/10 border-status-danger/20' : 'bg-surface-zinc border-[rgb(var(--fg-rgb))]/5'}`}><div className="flex items-center gap-3"><div className={`w-9 h-9 rounded-full flex items-center justify-center ${formData.isBlocked ? 'bg-status-danger text-white shadow-glow-sm' : 'bg-[rgb(var(--fg-rgb))]/5 text-disabled'}`}><Ban size={16} /></div><div><h4 className={`text-xs font-semibold ${formData.isBlocked ? 'text-status-danger-soft' : 'text-secondary'}`}>{formData.isBlocked ? 'Bloqueado' : 'Acceso Libre'}</h4><p className="text-[9px] text-disabled leading-tight">Evita ventas accidentales.</p></div></div><button type="button" onClick={() => setFormData(prev => ({ ...prev, isBlocked: !prev.isBlocked }))} className={`w-10 h-5.5 rounded-full relative transition-colors duration-300 border ${formData.isBlocked ? 'bg-status-danger border-status-danger' : 'bg-zinc-800 border-zinc-700'}`}><div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-md transition-transform duration-300 ${formData.isBlocked ? 'translate-x-4' : ''}`} /></button></div>
            ...
            <div className="pt-4 border-t border-[rgb(var(--fg-rgb))]/5 flex justify-end"><button type="submit" className="btn-primary px-12 h-11 lg:h-12 rounded-md text-xs font-semibold uppercase tracking-widest flex items-center gap-2"><Check size={18} strokeWidth={3} /> {initialData ? 'Actualizar' : 'Registrar'} Contacto</button></div>
          </form>
        </div>
      </Modal>
