          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                ...
                <div className={`p-4 rounded-md border flex items-center justify-between transition-all ${formData.isBlocked ? 'bg-red-500/10 border-red-500/20' : 'bg-surface-zinc border-white/5'}`}><div className="flex items-center gap-3"><div className={`w-9 h-9 rounded-full flex items-center justify-center ${formData.isBlocked ? 'bg-red-500 text-white shadow-glow-sm' : 'bg-white/5 text-zinc-500'}`}><Ban size={16} /></div><div><h4 className={`text-xs font-semibold ${formData.isBlocked ? 'text-red-400' : 'text-zinc-300'}`}>{formData.isBlocked ? 'Bloqueado' : 'Acceso Libre'}</h4><p className="text-[9px] text-zinc-500 leading-tight">Evita ventas accidentales.</p></div></div><button type="button" onClick={() => setFormData(prev => ({ ...prev, isBlocked: !prev.isBlocked }))} className={`w-10 h-5.5 rounded-full relative transition-colors duration-300 border ${formData.isBlocked ? 'bg-red-500 border-red-500' : 'bg-zinc-800 border-zinc-700'}`}><div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow-md transition-transform duration-300 ${formData.isBlocked ? 'translate-x-4' : ''}`} /></button></div>
