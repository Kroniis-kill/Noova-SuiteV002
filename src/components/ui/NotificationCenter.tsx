  ...
  const NotificationDetail = ({ n }: { n: AppNotification }) => {
    ...
    return (
      <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-300">
        ...
        <div className="bg-surface-3 border border-white/10 rounded-xl p-6 shadow-xl relative overflow-hidden">
          ...
          <div className="flex items-start gap-4 mb-6 relative z-10">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap gap-2 mb-1.5">
                ...
                <span className="px-2 py-0.5 rounded-full bg-white/5 text-zinc-400 text-[9px] font-black uppercase tracking-widest border border-white/10">{n.type}</span>
              </div>
              <h3 className="text-lg font-black text-white leading-tight">{n.title}</h3>
              <p className="text-[10px] text-zinc-500 font-mono mt-1 uppercase tracking-wider">{new Date(n.date).toLocaleString()}</p>
            </div>
          </div>

          <div className="bg-black/20 rounded-lg p-4 border border-white/5 mb-6 relative z-10">
          ...
          {(client || account) && (
            <div className="bg-white/5 rounded-lg p-4 border border-white/5 mb-6 animate-in fade-in zoom-in-95 duration-500">
              <h4 className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] mb-3">Información Relevante</h4>
              {client && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-500 font-mono">{client.phone}</p>
                  </div>
                </div>
              )}
              {account && (
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-zinc-800 flex items-center justify-center text-zinc-400"><Package size={18} /></div>
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-white truncate">{account.email}</p>
                    <p className="text-[10px] text-zinc-500 font-mono uppercase">Vence: {formatDate(account.endDate)}</p>
                  </div>
                </div>
              )}
            </div>
       ...
       <div className="flex flex-col h-[75vh] md:h-[600px] pt-1">
          ...
          {!selectedNotification && (
            <div className="space-y-4 shrink-0 mb-4 animate-in fade-in slide-in-from-top-2 duration-300">
               ...
               <div className="flex bg-black/20 p-1 rounded-md border border-white/5 overflow-x-auto no-scrollbar">
                  {[
                    ...
                    return (
                      <button 
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-sm whitespace-nowrap text-[11px] font-semibold uppercase transition-all ${
                          isActive 
                            ? 'bg-surface-3 text-white shadow-lg border border-white/10' 
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start mb-0.5">
                                  <h4 className={`text-xs font-semibold truncate pr-2 ${item.priority === 'high' ? 'text-red-100' : 'text-white'}`}>{item.title}</h4>
                                  <span className="text-[9px] text-zinc-500 whitespace-nowrap font-mono bg-white/5 px-1.5 py-0.5 rounded uppercase tracking-tighter shrink-0">{new Date(item.date).toLocaleDateString(undefined, { day: '2-digit', month: 'short' })}</span>
                                </div>
