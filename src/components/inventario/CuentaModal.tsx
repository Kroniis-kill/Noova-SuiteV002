    ...
    <Modal isOpen={isOpen} onClose={onClose} title={title} zIndex={70000}>
      <div className="flex flex-col h-[60vh] md:h-[400px] pt-1">
        <div className="relative mb-4 shrink-0 px-1">
           <Search size={20} className="absolute left-6 top-1/2 -translate-y-1/2 text-disabled" />
           <input 
             value={search} onChange={(e) => setSearch(e.target.value)}
             placeholder={placeholder}
             className="w-full bg-surface-sunken rounded-lg pl-12 pr-10 h-12 text-sm text-primary outline-none border border-[rgb(var(--fg-rgb))]/10 focus:border-brand-primary/60 transition-all placeholder:text-faint font-medium"
             autoFocus
           />
           {search && <button onClick={() => setSearch('')} className="absolute right-6 top-1/2 -translate-y-1/2 text-disabled hover:text-primary p-1"><X size={16} /></button>}
        </div>
...
const CuentaModal: React.FC<CuentaModalProps> = ({ isOpen, onClose, onSubmit, initialData, serviceId, services }) => {
  ...
  const styles = {
    sectionLabel: "text-[10px] font-bold text-faint uppercase tracking-[0.15em] mb-3 block ml-1",
    inputContainer: "relative flex items-center bg-surface-zinc border border-[rgb(var(--fg-rgb))]/5 rounded-md h-[52px] transition-all focus-within:border-brand-primary/40",
    input: "w-full h-full bg-transparent text-sm text-primary placeholder:text-faint px-4 outline-none font-medium",
    iconElement: "absolute left-4 text-faint pointer-events-none",
    cardDark: "bg-surface-sunken border border-[rgb(var(--fg-rgb))]/[0.03] rounded-xl p-5 shadow-inner",
  ...
  return (
    <>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-1">
          ...
          <div className="space-y-1">
            ...
            >
                ...
                <div className={`${styles.input} pl-12 flex items-center justify-between`}>
                    <span className={formData.serviceId ? "text-primary font-bold" : "text-faint"}>
          ...
          <div className="space-y-1">
            ...
            <div onClick={() => setIsProviderSearchOpen(true)} className={`${styles.inputContainer} cursor-pointer hover:bg-surface-3`}>
                ...
                <div className={`${styles.input} pl-12 flex items-center justify-between pr-4`}>
                    <span className={formData.providerId ? "text-primary font-bold" : "text-faint"}>
                        {selectedProvider ? selectedProvider.name : "Seleccionar..."}
                    </span>
                    <div className="flex items-center gap-2">
                        {formData.providerId && <X size={16} className="text-faint hover:text-primary" onClick={(e) => { e.stopPropagation(); setFormData({...formData, providerId: ''}); }} />}
                    </div>
                </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className={styles.inputContainer}>
                ...
                <button type="button" onClick={handleGeneratePassword} className="absolute right-3 p-2 text-faint hover:text-primary transition-colors">
                    <RefreshCw size={18} />
                </button>
            </div>
          </div>
                ...
                {profiles.map((prof, idx) => (
                    <div key={idx} className="flex gap-2">
                        <div className="flex-1 flex items-center bg-surface-zinc border border-[rgb(var(--fg-rgb))]/5 rounded-md h-[52px] overflow-hidden">
                            <div className="w-12 h-full flex items-center justify-center bg-black/20 border-r border-[rgb(var(--fg-rgb))]/5 text-faint text-xs font-semibold font-mono">
                                {idx + 1}
                            </div>
                            <input 
                                value={prof.name} 
                                ...
                                className="w-full h-full bg-transparent px-4 text-sm text-primary outline-none font-medium"
                            />
                        </div>
                        <div className="w-[100px] flex items-center bg-surface-zinc border border-[rgb(var(--fg-rgb))]/5 rounded-md h-[52px]">
                            <input 
                                ...
                                placeholder="PIN" 
                                className="w-full h-full bg-transparent px-4 text-center text-sm text-primary font-mono outline-none"
                                inputMode="numeric"
                            />
                        </div>
                    </div>
          ...
          <div className="bg-surface-1 border border-[rgb(var(--fg-rgb))]/[0.04] rounded-xl p-5 flex items-center justify-between shadow-inner">
             <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-md bg-surface-3 flex items-center justify-center text-brand-primary border border-[rgb(var(--fg-rgb))]/5 shadow-sm">
                   <RotateCcw size={22} className={formData.autoRenewal ? 'animate-spin-slow' : ''} />
                </div>
                <div>
                   <h4 className="text-sm font-bold text-primary leading-tight">Renovación Automática</h4>
                   <p className="text-[10px] text-disabled font-medium mt-0.5">Se extenderá 1 mes al vencer</p>
                </div>
             </div>
             <button 
                type="button" 
          ...
          <div className="space-y-1">
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="flex items-center bg-surface-zinc border border-[rgb(var(--fg-rgb))]/5 rounded-lg h-[52px] overflow-hidden">
                  <div className="px-4 h-full flex items-center justify-center bg-black/20 border-r border-[rgb(var(--fg-rgb))]/5 text-[9px] font-black text-disabled uppercase tracking-widest">
                     Mes
                  </div>
                  <input type="number" value={months} onChange={e => setMonths(e.target.value)} className="w-full h-full bg-transparent text-center text-sm text-primary font-bold outline-none" placeholder="0" />
               </div>
               <div className="flex items-center bg-surface-zinc border border-[rgb(var(--fg-rgb))]/5 rounded-lg h-[52px] overflow-hidden">
                  <div className="px-4 h-full flex items-center justify-center bg-black/20 border-r border-[rgb(var(--fg-rgb))]/5 text-[9px] font-black text-disabled uppercase tracking-widest">
                     Día
                  </div>
                  <input type="number" value={days} onChange={e => setDays(e.target.value)} className="w-full h-full bg-transparent text-center text-sm text-primary font-bold outline-none" placeholder="0" />
               </div>
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <FileText size={18} className="absolute left-4 top-4 text-faint pointer-events-none" />
              <textarea 
                value={formData.notes || ''} 
                onChange={e => setFormData({...formData, notes: e.target.value})} 
                placeholder="Escribe detalles adicionales sobre esta cuenta..." 
                className="w-full bg-surface-zinc border border-[rgb(var(--fg-rgb))]/5 rounded-md pl-12 pr-4 pt-4 pb-4 text-sm text-primary outline-none focus:border-brand-primary/40 transition-all placeholder:text-faint min-h-[120px] font-medium resize-none leading-relaxed"
              />
            </div>
          </div>
          ...
          <div className="pt-4 pb-4">
             <button 
                type="submit" 
                className="w-full h-[60px] bg-surface-3 border border-[rgb(var(--fg-rgb))]/10 text-primary rounded-lg font-bold text-sm flex items-center justify-center gap-3 active:scale-95 transition-all shadow-xl"
             >
      ...
      <SearchListModal<Service>
        ...
        renderItem={(s) => (
          <div className="w-full flex items-center gap-4 p-4 rounded-lg bg-surface-3 border border-[rgb(var(--fg-rgb))]/5 hover:border-brand-primary/40 transition-all text-left">
              <div className="w-11 h-11 rounded-md bg-surface-sunken flex items-center justify-center text-faint border border-[rgb(var(--fg-rgb))]/5 overflow-hidden shrink-0">
              ...
              <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[9px] text-faint font-black uppercase tracking-widest">{s.type.replace('_', ' ')}</p>
              </div>
      ...
      <SearchListModal<Provider>
        ...
        renderItem={(p) => (
          <div className="w-full flex items-center gap-4 p-4 rounded-lg bg-surface-3 border border-[rgb(var(--fg-rgb))]/5 hover:border-brand-primary/40 transition-all text-left">
              <div className="w-11 h-11 rounded-md flex items-center justify-center text-primary font-semibold text-xs shrink-0" style={{ backgroundColor: p.color }}>
                {p.name.substring(0,2).toUpperCase()}
              </div>
              <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-primary truncate">{p.name}</p>
                  <p className="text-[10px] text-faint font-mono">{p.whatsapp}</p>
              </div>
              <ChevronRight size={18} className="text-faint" />
          </div>
        )}
