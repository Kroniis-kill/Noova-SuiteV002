...
const CuentaModal: React.FC<CuentaModalProps> = ({ isOpen, onClose, onSubmit, initialData, serviceId, services }) => {
  ...
  const styles = {
    sectionLabel: "text-[10px] font-bold text-text-faint uppercase tracking-[0.15em] mb-3 block ml-1",
    ...
    input: "w-full h-full bg-transparent text-sm text-text-primary placeholder:text-text-faint px-4 outline-none font-medium",
    iconElement: "absolute left-4 text-text-faint pointer-events-none",
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-1">
          ...
          <div className="space-y-1">
            ...
            >
                ...
                <div className={`${styles.input} pl-12 flex items-center justify-between`}>
                    <span className={formData.serviceId ? "text-text-primary font-bold" : "text-text-faint"}>
          ...
          <div className="space-y-1">
            ...
            <div onClick={() => setIsProviderSearchOpen(true)} className={`${styles.inputContainer} cursor-pointer hover:bg-surface-3`}>
                ...
                <div className={`${styles.input} pl-12 flex items-center justify-between pr-4`}>
                    <span className={formData.providerId ? "text-text-primary font-bold" : "text-text-faint"}>
                        {selectedProvider ? selectedProvider.name : "Seleccionar..."}
                    </span>
                    <div className="flex items-center gap-2">
                        {formData.providerId && <X size={16} className="text-text-faint hover:text-text-primary" onClick={(e) => { e.stopPropagation(); setFormData({...formData, providerId: ''}); }} />}
                    </div>
                </div>
            </div>
          </div>
          ...
          <div className="bg-surface-1 border border-[rgb(var(--fg-rgb))]/[0.04] rounded-xl p-5 flex items-center justify-between shadow-inner">
             <div className="flex items-center gap-4">
                ...
                <div>
                   <h4 className="text-sm font-bold text-text-primary leading-tight">Renovación Automática</h4>
                   <p className="text-[10px] text-text-disabled font-medium mt-0.5">Se extenderá 1 mes al vencer</p>
                </div>
             </div>
             <button 
                type="button" 
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <FileText size={18} className="absolute left-4 top-4 text-text-faint pointer-events-none" />
              <textarea 
                value={formData.notes || ''} 
                onChange={e => setFormData({...formData, notes: e.target.value})} 
                placeholder="Escribe detalles adicionales sobre esta cuenta..." 
                className="w-full bg-surface-zinc border border-[rgb(var(--fg-rgb))]/5 rounded-md pl-12 pr-4 pt-4 pb-4 text-sm text-text-primary outline-none focus:border-brand-primary/40 transition-all placeholder:text-text-faint min-h-[120px] font-medium resize-none leading-relaxed"
              />
            </div>
          </div>
