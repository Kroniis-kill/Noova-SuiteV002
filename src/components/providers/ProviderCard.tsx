       ...
       <div className="flex justify-between items-start mb-3 relative z-10">
          <div className="flex items-center gap-3">
             ...
             <div className="min-w-0">
                <h3 className={`text-[15px] font-bold truncate leading-tight ${isActive ? 'text-primary' : 'text-secondary group-hover:text-primary'}`}>{provider.name}</h3>
                <div className="flex items-center gap-2 mt-1">
                    <span className="text-[11px] text-disabled font-mono bg-[rgb(var(--fg-rgb))]/5 px-1.5 rounded border border-[rgb(var(--fg-rgb))]/5 truncate">{provider.whatsapp}</span>
          ...
          <div className="flex gap-1"><button onClick={handleEditClick} className="w-8 h-8 rounded-full bg-[rgb(var(--fg-rgb))]/5 hover:bg-[rgb(var(--fg-rgb))]/10 flex items-center justify-center text-muted hover:text-primary transition-colors border border-[rgb(var(--fg-rgb))]/5"><Edit2 size={14} /></button></div>
       </div>

       <div className="flex items-center justify-between pt-3 border-t border-[rgb(var(--fg-rgb))]/5 relative z-10">
          <div className="flex items-center gap-2"><div className="flex items-center justify-center w-6 h-6 rounded-full bg-status-info/10 text-status-info-soft border border-status-info/20"><Layers size={12} /></div><span className="text-[11px] font-medium text-muted"><strong className="text-secondary">{accountCount}</strong> Items</span></div>
          <div className="flex gap-2">
             ...
             <div className="w-8 h-8 flex items-center justify-center text-faint"><ChevronRight size={16} /></div>
          </div>
       </div>
    </motion.div>
  );
