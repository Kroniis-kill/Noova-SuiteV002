...
export const BusinessSettings = () => {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
             <div className="flex bg-surface-1 p-1.5 rounded-lg border border-white/[0.08] w-full mb-4 shadow-sm">
                 {tabs.map(tab => (
                     <button
                        key={tab.id}
                        ...
                        className={`flex-1 px-4 py-2.5 rounded-md text-xs font-semibold transition-all text-center ${
                            activeTab === tab.id
                            ? 'bg-surface-4 text-white shadow-sm border border-white/5'
                    ...
                    {activeTab === 'finances' && (
                        <motion.div key="finances" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-6">
                            <div className="flex items-center gap-2 mb-2 text-brand-primary">
                                <CreditCard size={18} />
                                <h3 className="font-bold text-white text-sm">Configuración de Moneda</h3>
                            </div>
                            ...
                            <div className="mt-2 pt-4 border-t border-white/5">
                                <div className={styles.toggleContainer}>
                                    <div>
                                        <p className="text-sm font-bold text-white">Compras como Costo</p>
                                        <p className="text-[10px] text-zinc-500">Restar stock de la ganancia</p>
                                    </div>
                    ...
                    {activeTab === 'sales' && (
                        <motion.div key="sales" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-6">
                            <div className="flex items-center gap-2 mb-2 text-brand-accent">
                                ...
                                <h3 className="font-bold text-white text-sm">Preferencias de Venta</h3>
                            </div>
                                    ...
                                    {[1, 2, 3, 5].map(day => (
                                        <button
                                            ...
                                            className={`h-12 rounded-md font-bold text-sm border transition-all ${
                                                ...
                                                : 'bg-surface-sunken border-white/10 text-zinc-500 hover:border-white/20'
                                            }`}
                                        >
                                            {day} Días
                                        </button>
                            ...
                            <div className={styles.toggleContainer}>
                                <div>
                                    <p className="text-sm font-bold text-white">Generar PIN Automático</p>
                                    <p className="text-[10px] text-zinc-500">Asignar PIN aleatorio al vender</p>
                                </div>
                    ...
                    {activeTab === 'categories' && (
                        <motion.div key="categories" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="space-y-6">
                            ...
                            <div className="flex gap-2">
                                ...
                                <button onClick={handleAddCategory} className="w-[52px] h-[52px] rounded-md bg-surface-4 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-colors shrink-0">
                                    <Plus size={20} />
                                </button>
                            </div>
                            ...
                            <div className="space-y-2 max-h-[300px] overflow-y-auto custom-scrollbar pr-1">
                                {expenseCategories.length === 0 && (
                                    <p className="text-center text-zinc-500 text-xs py-8">No hay categorías personalizadas.</p>
                                )}
                                {expenseCategories.map(cat => (
                                    <div key={cat.id} className="flex items-center justify-between p-4 bg-surface-sunken border border-white/5 rounded-md group">
